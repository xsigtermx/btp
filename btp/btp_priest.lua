...
function PriestBuff()
    ...
    while true do
       ...
       if (strfind(spellName, "Inner Fire")) then
           hasInnerFire = true;
       end
...
function btp_priest_dps_new(unit)
	...
	if(cur_health/cur_health_max < .65) then
	...
	if(not UnitAffectingCombat("player")) then
		...
		if(not btp_priest_is_fortitude("player")) then
			if(btp_cast_spell("Power Word: Fortitude")) then return true; end
		end
		...
		if(not btp_priest_is_innerfire()) then
	...
	if(cur_health/cur_health_max < .55) then
	...
	if(my_swp == false) then
		if(btp_cast_spell_on_target("Shadow Word: Pain", unit)) then 
			return true 
		end;
	end

	-- shield our self if we are in combat
        if (not btp_priest_is_pws() and UnitAffectingCombat("player"))  then
	    has_pws, my_pws, num_pws = btp_check_debuff("AshesToAshes", "player");
	    if(has_pws == false) then
		if(btp_cast_spell("Power Word: Shield")) then return true; end
            end
	end

	-- check if our health is low if so heal ourself
	if(cur_health/cur_health_max < .95) then
	...
	if(cur_health/cur_health_max < .25) then
    ...
    --
	if(cur_health/cur_health_max < .75) then
	...
	-- if cooldown is up use
	if(btp_cast_spell_on_target("Devouring Plague", unit)) then return true end

	-- if we are in a party be a good shadow priest and give people there mana/health
	if(GetNumPartyMembers() >= 2 and UnitHealth(unit)/UnitHealthMax(unit) > .62) then

		-- cast vamperic embrace if it's not up
		has_vemb, my_vemb, num_vemb = btp_check_debuff("UnsummonBuilding", unit);
		if(not my_vemb) then
		...
		if(not my_vemb) then
			if(btp_cast_spell_on_target("Vampiric Touch", unit)) then 
				return true 
			end;
		end
	end
	...
	if(not btp_priest_is_fearward()) then
		if(btp_cast_spell_on_target("Fear Ward")) then return true; end
	end

	-- always mind blast first since it's on cooldown
	...
	-- we dont have mind flay so just smite cause everything else is in cooldown
...
function btp_priest_is_swp(unit)
	if(not unit) then  unit = "player"; end
	if(btp_check_buff("ShadowWordPain", unit)) then return true; end
	return false
end
...
--     if(lowest_percent ~= nil and lowest_percent ~= false) then
--         btp_frame_debug("lowest_percent: " .. lowest_percent);
--     end

--     if(lowest_health ~= nil) then
--         btp_frame_debug("lowest_health: " .. lowest_health);
--     end

--     if(lowest_target ~= nil) then
--         btp_frame_debug("lowest_target: " .. lowest_target);
--     end
    -- local lowest_target, lowest_health, lowest_percent = btp_health_status_quick();
    -- there is no one to heal if health status does not return a target
--     if(not lowest_target) then 
--         btp_frame_debug("nothing to heal");
--         stopMoving = false;
--         return false; 
--     end
...
function btp_priest_heal_crit(cur_percent, cur_health, cur_player)
    ...
    -- btp_frame_debug("NEED CRIT " .. cur_player .. " " ..
    --                 UnitName(cur_player));
...
function btp_priest_heal_large(cur_percent, cur_health, cur_player)
    ...
    -- btp_frame_debug("NEED LARGE " .. cur_player .. " " ..
    --                 UnitName(cur_player));
...
function btp_priest_heal_medium(cur_percent, cur_health, cur_player)
    ...
    -- btp_frame_debug("NEED MEDIUM " .. cur_player .. " " ..
    --                 UnitName(cur_player));
...
function btp_priest_heal_small(cur_percent, cur_health, cur_player)
    ...
    -- btp_frame_debug("NEED SMALL " .. cur_player .. " " ..
    --                 UnitName(cur_player));
   	if(btp_priest_is_sol()) then 
		if(btp_cast_spell_on_target("Flash Heal", cur_player)) then return true; end;
	end
...
function btp_priest_is_pws(unit)
	if(not unit) then  unit = "player"; end
	if(btp_check_buff("AshesToAshes", unit)) then return true; end
